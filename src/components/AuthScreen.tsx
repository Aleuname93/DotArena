...
export default function AuthScreen({ auth }: Props) {
  const { step, error, sending, sendOtp, verifyOtp, setDisplayName, resetToEmail } = auth
    ...
    <div className="crt min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-8">
        ...
        <div className="w-full bg-black p-7"
          ...
          {step === 'unauthenticated' && (
            <>
              ...
              <div className="relative mb-4"
                style={{ border: '2px solid #00ff41', boxShadow: '2px 2px 0 #003a0f' }}>
                <input
                  autoFocus
                  type="email"
                  value={email}
                  onChange={e => setEmail(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && email && sendOtp(email)}
              ...
              <button onClick={() => email && sendOtp(email)} disabled={!email || sending}
                className="w-full py-4 font-pixel text-[10px] tracking-wider transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-30"
                style={{ backgroundColor: '#00ff41', color: '#000', border: '2px solid #00ff41', boxShadow: '4px 4px 0 #003a0f' }}>
                {sending ? '...' : '▶ ENVIAR CÓDIGO'}
              </button>
            </>
          )}
          ...
          {step === 'otp_sent' && (
            <>
              <p className="text-[#ffdd00] text-[8px] font-pixel mb-1 text-center pixel-blink">★ EMAIL ENVIADO ★</p>

              {/* Envelope animation */}
              <div className="flex justify-center py-4">
                <div className="text-4xl" style={{ animation: 'blink 2s step-end infinite' }}>📨</div>
              </div>
              ...
              <p className="text-[#00ff41]/40 text-[6px] font-pixel mb-6 text-center">
                O LINK ABRE O JOGO DIRETO NO SEU NAVEGADOR
              </p>

              <div className="py-3 mb-4 text-center"
                style={{ border: '1px solid #00ff4130' }}>
                <p className="text-[#00ff41]/60 text-[7px] font-pixel">AGUARDANDO LOGIN...</p>
                <div className="flex justify-center gap-1 mt-2">
                  {[0,1,2].map(i => (
                    <div key={i} className="w-1.5 h-1.5 bg-[#00ff41] rounded-full animate-bounce"
              ...
              <button onClick={resetToEmail}
                className="w-full py-3 font-pixel text-[8px] text-white/30 hover:text-white/60 transition-colors"
          ...
          {/* ── LOADING ── */}
          {step === 'loading' && (
            <p className="text-[#00ff41] text-[8px] font-pixel text-center py-4 pixel-blink">CARREGANDO...</p>
          )}
        </div>

        <p className="text-white/15 text-[6px] font-pixel text-center">© 2024 DOT ARENA • INSERT COIN</p>
      </div>
    </div>
  )
}
